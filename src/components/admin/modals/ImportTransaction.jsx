...
const ImportTransaction = ({ show, setShow, fetchData }) => {
  ...
  const validateImportData = ({ productId, quantity, price }) => {
    ...
    if (!price) {
      newError.errorPrice = "Vui lòng điền giá nhập";
    } else if (isNaN(price) || Number(price) <= 0) {
      newError.errorPrice = "Giá nhập phải là số lớn hơn 0.";
    }
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      if (response?.code === 201 && response?.result) {
        ...
        toast.error("Lội nhập hàng");
      }
    } catch (error) {
      console.log(error);
      toast.error("Lỗi kho nhập hàng - Vui lòng kiểm tra console.log");
    }
  };
